...
app.controller('UaCtrl', function($scope) {
    ...
    chrome.storage.local.get({'ua': true}, function(result) {
        ...
        $scope.$watch('ua', function() {
            chrome.storage.local.set({'ua': $scope.ua}, function() {
                if ( $scope.ua === true ) chrome.extension.getBackgroundPage().loadga();
                if ( $scope.ua === false ) chrome.extension.getBackgroundPage().unloadga();
            });
        });

    });

    //  send analytics on ua change
    $scope.toggleUA = function() {
        chrome.extension.getBackgroundPage().ga('send', 'event', {
            eventCategory: 'State',
            eventAction: 'ua',
            eventLabel: ( $scope.ua === true ) ? 'true' : 'false'
        });
    };
...
chrome.extension.getBackgroundPage().ga('send', 'pageview');
